		...
		public void Seat() { 					//예약메소드
			...
			while(true) { //조건이 true일 경우 계속해서 반복
				...
				if(hospitals == 1) {
					...
					rr.h1.month.calendar(calendar.get(Calendar.MONTH)+1);
					...
					for (int i = 0; i<rr.h1.month.day.size(); i++) {
						
						if(rr.h1.month.day.get(i).getDay().equals(res_day.substring(3))){
							if(rr.h1.month.day.get(i).isRes_possibility()) {
								
								rr.h1.month.day.get(i).setRes_possibility(false);
								//모든입력완료 후 리스트로 저장
								dayNum = res_day;
					...
					rr.h2.month.calendar(calendar.get(Calendar.MONTH)+1);
					
				}else {						// 이화여자 병원
					rr.h3.month.calendar(calendar.get(Calendar.MONTH)+1);
				}
